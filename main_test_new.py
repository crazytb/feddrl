...
def test_agent(env, agent, episodes=100, render=False, agent_name="Agent", verbose=True):
    ...
    for episode in range(episodes):
        ...
        # Only print progress for verbose mode and less frequently for multiple environments
        if verbose and episode % max(20, episodes//5) == 0:
            print(f"    {agent_name} Episode {episode+1}/{episodes}: Reward = {episode_reward:.1f}, Length = {episode_length}")
...
def test_models_on_all_envs(model_dir, test_envs, test_episodes=100, env_type="training"):
    """Test all models on ALL environments (for fair comparison on training environments)"""
    model_files = sorted(glob.glob(os.path.join(model_dir, "agent_*.pth")))
    results = []
    
    print(f"\nTesting models from: {model_dir}")
    print(f"Using {env_type} environments - ALL AGENTS ON ALL ENVIRONMENTS")
    print(f"Found {len(model_files)} model files")
    print(f"Testing on {len(test_envs)} environments each")
    
    for i, model_file in enumerate(model_files):
        print(f"\n--- Testing Agent {i+1} on ALL {len(test_envs)} environments ---")
        ...
        for env_idx, env in enumerate(test_envs):
            print(f"  Testing on Environment {env_idx+1}/{len(test_envs)}: "
                  f"max_comp_units={env.max_comp_units}, "
                  f"agent_velocities={env.agent_velocities}")
            
            # Test the agent on this environment
            env_result = test_agent(env, agent, episodes=test_episodes//len(test_envs), 
                                  agent_name=f"Agent {i+1}", render=False, verbose=False)
            
            # Accumulate results from all environments
            agent_total_rewards.extend(env_result['all_rewards'])
            agent_total_lengths.extend(env_result['all_lengths'])
        
        # Calculate overall statistics for this agent across all environments
        result = {
            'mean_reward': np.mean(agent_total_rewards),
            'std_reward': np.std(agent_total_rewards),
            'mean_length': np.mean(agent_total_lengths),
            'std_length': np.std(agent_total_lengths),
            'all_rewards': agent_total_rewards,
            'all_lengths': agent_total_lengths,
            'agent_id': i + 1,
            'model_file': model_file,
            'env_type': env_type,
            'num_environments': len(test_envs)
        }
        results.append(result)
        
        print(f"Agent {i+1} Overall Results - Mean Reward: {result['mean_reward']:.2f} ± {result['std_reward']:.2f}, "
              f"Mean Length: {result['mean_length']:.2f} ± {result['std_length']:.2f}")
    
    return results

def test_models_on_individual_envs(model_dir, test_envs, test_episodes=100, env_type="generalization"):
    """Test each model on its corresponding environment (for generalization testing)"""
    model_files = sorted(glob.glob(os.path.join(model_dir, "agent_*.pth")))
    results = []
    
    print(f"\nTesting models from: {model_dir}")
    print(f"Using {env_type} environments - EACH AGENT ON ITS OWN ENVIRONMENT")
    print(f"Found {len(model_files)} model files")
    ...
    for i, model_file in enumerate(model_files):
        ...
        if i < len(test_envs):
            ...
            # Test the agent
            result = test_agent(env, agent, episodes=test_episodes, agent_name=f"Agent {i+1}", verbose=True)
            result['agent_id'] = i + 1
...
def compare_results(independent_results, federated_results, env_type="training"):
    ...
    if env_type == "training":
        print(f"COMPARISON RESULTS (TRAINING ENVIRONMENTS - ALL AGENTS TESTED ON ALL ENVIRONMENTS)")
    else:
        print(f"COMPARISON RESULTS (GENERALIZATION ENVIRONMENTS - EACH AGENT ON ITS OWN ENVIRONMENT)")
    print("="*80)
...
def main():
    ...
    # Test on training environments (if available) - ALL AGENTS ON ALL TRAINING ENVIRONMENTS
    if training_envs:
        print("\n" + "="*80)
        print("TESTING ON TRAINING ENVIRONMENTS (ALL AGENTS ON ALL ENVIRONMENTS)")
        print("="*80)
        
        independent_results_train = test_models_on_all_envs(independent_dir, training_envs, test_episodes, "training")
        federated_results_train = test_models_on_all_envs(federated_dir, training_envs, test_episodes, "training")
    ...
    # Test on new environments (generalization) - EACH AGENT ON ITS OWN NEW ENVIRONMENT
    print("\n" + "="*80)
    print("TESTING ON NEW ENVIRONMENTS (EACH AGENT ON ITS OWN ENVIRONMENT)")
    print("="*80)
    
    independent_results_gen = test_models_on_individual_envs(independent_dir, generalization_envs, test_episodes, "generalization")
    federated_results_gen = test_models_on_individual_envs(federated_dir, generalization_envs, test_episodes, "generalization")
